...
{
    @Test
    public void testDatalake() {
        String accountName = SdkContext.randomResourceName("ac", 10);

        NameAvailabilityInformationInner checkNameGetResponse =
                dataLakeStoreManager.dataLakeStoreAccounts().checkNameAvailability(
        ...
        DataLakeStoreAccount account = dataLakeStoreManager.dataLakeStoreAccounts().define(accountName)
                .withRegion(environmentLocation)
                .withExistingResourceGroup(resourceGroupName)
                .withTag("testKey", "testValue")
                .create();
        System.out.printf(account.inner().currentTier().toString());

        Assert.assertEquals(environmentLocation, account.region());
        Assert.assertEquals("Microsoft.DataLakeStore/accounts", account.type());
        Assert.assertNotNull(account.id());
